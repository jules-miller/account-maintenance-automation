...
class Validation:
	...
	def ValidateSqlUsers(logger: logging.Logger, value: str) -> bool:
		...
		Example:
		Pass: 'user1@domain','user''2@domain' 
		Pass: 'user1@domain'
		Fail: 'user1@domain','user''2@domain','user3@domain',
		Fail: 'user1@domain

		Pass: 'user1@domain user2@domain'
		Ideally, the above string would not pass.  However, due to the way the string is constructed, it will always pass.  
		Including extra regex to require @domain would only fix this is certain cases.  However, this isn't desirable because local api accounts \
		without the @domain suffix can be passed to the query.  In the case of the name conversion queires, SQL will just ignore it, \
